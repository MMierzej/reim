...
instance Eq Regex where
    ...
    Group id lb re == Group ie lc rf = lb == lc && re == rf
    _              == _              = False
...
instance Show Regex where
    ...
    show = stringify
        where
            ...
            showStruct (Group id lb re) = "Group " ++ show id ++ " \"" ++ lb ++ "\" (" ++ show re ++ ")"
            ...
            stringify (Group _ "" re) = "("               ++ show re ++ ")"
            stringify (Group _ lb re) = "(<" ++ lb ++ ">" ++ show re ++ ")"
...
simpl (Group id lb re) = Group id lb (simpl re)
simpl re               = re
...
group ('(':s) = case auxparse False s' of
    Just (re, glue, ')':s'') -> Just (Eps `glue` Group 0 name re, Cat, s'')
    _ -> Nothing
    where
